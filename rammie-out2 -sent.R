...
#==========================
myDir <- "/local/zck07apu/Documents/GitLab/rammie_comparison/scripts/NB/2x"
years=7
...
for(i in 1:nsim){
     ...
     #mu=exp(h2(N=N,k=4,k2=1,alpha=1.5,beta=0,gama1=0.1,gama2=0.1,gama3=1.8,gama4=0.1,shift=-85)+2))
...
#############################
#############################
# FIT EARS ALGO TO DATA AND INSERT THE ALARMS IN THE ALARM DATA FRAMES
#############################
#############################
...
for(j in 1:nsim){
     for(i in (49*7):1){
          nu=(alarmall1[nrow(alarmall1)-i+1,j]==1 & 
                   simulatedoutbreak1[nrow(simulatedoutbreak1)-i+1,j]==0)+nu
     }
}
fpr[1]=nu/sum(simulatedoutbreak1==0)  
...
fpr[2]=nu/sum(simulatedoutbreak2==0)   
...
fpr[3]=nu/sum(simulatedoutbreak3==0)   
...
fpr[4]=nu/sum(simulatedoutbreak4==0) 
...
fpr[5]=nu/sum(simulatedoutbreak5==0)   
...
fpr[6]=nu/sum(simulatedoutbreak6==0)   
...
fprseas[1]=nu/sum(simulatedzseasoutbreak6==0)   
...
fpr[7]=nu/sum(simulatedoutbreak7==0)   
...
fprseas[2]=nu/sum(simulatedzseasoutbreak7==0) 
...
fpr[8]=nu/sum(simulatedoutbreak8==0)   
...
fpr[9]=nu/sum(simulatedoutbreak9==0)   
...
fpr[10]=nu/sum(simulatedoutbreak10==0)   
...
fpr[11]=nu/sum(simulatedoutbreak11==0)   
...
fpr[12]=nu/sum(simulatedoutbreak12==0)   
...
fpr[13]=nu/sum(simulatedoutbreak13==0)  
...
fpr[14]=nu/sum(simulatedoutbreak14==0)   
...
fpr[15]=nu/sum(simulatedoutbreak15==0)   
...
fpr[16]=nu/sum(simulatedoutbreak16==0)   
...
fprseas[3]=nu/sum(simulatedzseasoutbreak16==0)   
...
fpr[17]=nu/sum(simulatedoutbreak17==0)   
...
pod[1]=nu/sum(simulatedoutbreak1>0)
...
pod[2]=nu/sum(simulatedoutbreak2>0)
...
pod[3]=nu/sum(simulatedoutbreak3>0)
...
pod[4]=nu/sum(simulatedoutbreak4>0)
...
pod[5]=nu/sum(simulatedoutbreak5>0)
...
pod[6]=nu/sum(simulatedoutbreak6>0)
...
podseas[1]=nu/sum(simulatedzseasoutbreak6>0)
...
pod[7]=nu/sum(simulatedoutbreak7>0)
...
podseas[2]=nu/sum(simulatedzseasoutbreak7>0)
...
pod[8]=nu/sum(simulatedoutbreak8>0)
...
pod[9]=nu/sum(simulatedoutbreak9>0)
...
pod[10]=nu/sum(simulatedoutbreak10>0)
...
pod[11]=nu/sum(simulatedoutbreak11>0)
...
pod[12]=nu/sum(simulatedoutbreak12>0)
...
pod[13]=nu/sum(simulatedoutbreak13>0)
...
pod[14]=nu/sum(simulatedoutbreak14>0)
...
pod[15]=nu/sum(simulatedoutbreak15>0)
...
pod[16]=nu/sum(simulatedoutbreak16>0)
...
podseas[3]=nu/sum(simulatedzseasoutbreak16>0)
...
pod[17]=nu/sum(simulatedoutbreak17>0)
...
sensitivity[1]=nu/(nu+fpr[1]*sum(simulatedoutbreak1==0))
...
sensitivity[2]=nu/(nu+fpr[2]*sum(simulatedoutbreak2==0))
...
sensitivity[3]=nu/(nu+fpr[3]*sum(simulatedoutbreak3==0))
...
sensitivity[4]=nu/(nu+fpr[4]*sum(simulatedoutbreak4==0))
...
sensitivity[5]=nu/(nu+fpr[5]*sum(simulatedoutbreak5==0))
...
sensitivity[6]=nu/(nu+fpr[6]*sum(simulatedoutbreak6==0))
...
sensitivityseas[1]=nu/(nu+fprseas[1]*sum(simulatedzseasoutbreak6==0))
...
sensitivity[7]=nu/(nu+fpr[7]*sum(simulatedoutbreak7==0))
...
sensitivityseas[2]=nu/(nu+fprseas[2]*sum(simulatedzseasoutbreak7==0))
...
sensitivity[8]=nu/(nu+fpr[8]*sum(simulatedoutbreak8==0))
...
sensitivity[9]=nu/(nu+fpr[9]*sum(simulatedoutbreak9==0))
...
sensitivity[10]=nu/(nu+fpr[10]*sum(simulatedoutbreak10==0))
...
sensitivity[11]=nu/(nu+fpr[11]*sum(simulatedoutbreak11==0))
...
sensitivity[12]=nu/(nu+fpr[12]*sum(simulatedoutbreak12==0))
...
sensitivity[13]=nu/(nu+fpr[13]*sum(simulatedoutbreak13==0))
...
sensitivity[14]=nu/(nu+fpr[14]*sum(simulatedoutbreak14==0))
...
sensitivity[15]=nu/(nu+fpr[15]*sum(simulatedoutbreak15==0))
...
sensitivity[16]=nu/(nu+fpr[16]*sum(simulatedoutbreak16==0))
...
sensitivityseas[3]=nu/(nu+fprseas[3]*sum(simulatedzseasoutbreak16==0))
...
sensitivity[17]=nu/(nu+fpr[1]*sum(simulatedoutbreak17==0))
...
#specificity[1]=nu/sum(simulatedoutbreak1==0)
specificity[1]=nu/(nu+fpr[1]*sum(simulatedoutbreak1==0))
...
specificity[2]=nu/(nu+fpr[2]*sum(simulatedoutbreak2==0))
...
specificity[3]=nu/(nu+fpr[3]*sum(simulatedoutbreak3==0))
...
specificity[4]=nu/(nu+fpr[4]*sum(simulatedoutbreak4==0))
...
specificity[5]=nu/(nu+fpr[5]*sum(simulatedoutbreak5==0))
...
specificity[6]=nu/(nu+fpr[6]*sum(simulatedoutbreak6==0))
...
specificityseas[1]=nu/(nu+fprseas[1]*sum(simulatedzseasoutbreak6==0))
...
specificity[7]=nu/(nu+fpr[7]*sum(simulatedoutbreak7==0))
...
specificityseas[2]=nu/(nu+fprseas[2]*sum(simulatedzseasoutbreak7==0))
...
specificity[8]=nu/(nu+fpr[8]*sum(simulatedoutbreak8==0))
...
specificity[9]=nu/(nu+fpr[9]*sum(simulatedoutbreak9==0))
...
specificity[10]=nu/(nu+fpr[10]*sum(simulatedoutbreak10==0))
...
specificity[11]=nu/(nu+fpr[11]*sum(simulatedoutbreak11==0))
...
specificity[12]=nu/(nu+fpr[12]*sum(simulatedoutbreak12==0))
...
specificity[13]=nu/(nu+fpr[13]*sum(simulatedoutbreak13==0))
...
specificity[14]=nu/(nu+fpr[14]*sum(simulatedoutbreak14==0))
...
specificity[15]=nu/(nu+fpr[15]*sum(simulatedoutbreak15==0))
...
specificity[16]=nu/(nu+fpr[16]*sum(simulatedoutbreak16==0))
...
specificityseas[3]=nu/(nu+fprseas[3]*sum(simulatedzseasoutbreak16==0))
...
specificity[17]=nu/(nu+fpr[17]*sum(simulatedoutbreak17==0))
...
summary1=data.frame(fpr, pod, pod2, sensitivity, specificity, timeliness)
...
summary2=data.frame(fprseas, podseas, podseas2, sensitivityseas,
...
write.csv(summary1, file.path(myDir, "output", "summaryNB.csv"), 
          row.names=FALSE)


write.csv(summary2, file.path(myDir, "output", "summarySeasNB.csv"), 
          row.names=FALSE)
